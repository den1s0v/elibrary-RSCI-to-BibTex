...
- `true` — приводит к нормальному регистру авторов, названия, журнала/источника (с попыткой не менять аббревиатуры)
- `false` — оставляет исходный регистр из eLIBRARY

## Экспорт коллекции

На страницах eLIBRARY отображается панель скрипта с кнопками:
- `Экспорт JSON` — выгружает всю локальную коллекцию в JSON-файл.
- `Экспорт BibTeX` — выгружает все накопленные BibTeX-записи в единый `.bib` файл.
